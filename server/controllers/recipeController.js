...
const getSavedRecipes = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `SELECT id, spoonacular_recipe_id, title, image_url, summary, ready_in_minutes as readyInMinutes, servings
                FROM recipes
...
const deleteFavoriteRecipe = async (req, res) => {
    ...
    try {
        // select id from recipes where spoonacular_recipe_id = recipeId and then delete from favorites
        const result = await pool.query(
            `DELETE FROM favorites
                WHERE user_id = $1 AND recipe_id IN (SELECT id FROM recipes WHERE spoonacular_recipe_id = $2)`,
            [userId, recipeId]
        );
